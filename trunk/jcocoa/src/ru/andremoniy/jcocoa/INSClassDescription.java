package ru.andremoniy.jcocoa;

import ru.andremoniy.jcocoa.*;
import ns.NSArray;
import static ns.NSArray.*;
import ns.INSArray;
import static ns.INSArray.*;
import ns.NSClassDescription;
import static ns.NSClassDescription.*;
import ns.INSClassDescription;
import static ns.INSClassDescription.*;

import ns.NSObject;

public abstract class INSClassDescription extends NSObject {
	public NSArray attributeKeys() { return null;
};
	public NSArray toOneRelationshipKeys() { return null;
};
	public NSArray toManyRelationshipKeys() { return null;
};
	public String inverseForRelationshipKey(String relationshipKey) { return null;
};
	public NSClassDescription classDescription() { return null;
};
	public NSArray attributeKeys() { return null;
};
	public NSArray toOneRelationshipKeys() { return null;
};
	public NSArray toManyRelationshipKeys() { return null;
};
	public String inverseForRelationshipKey(String relationshipKey) { return null;
};
}
